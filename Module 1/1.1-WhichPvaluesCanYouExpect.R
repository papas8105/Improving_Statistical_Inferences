...
for(i in 1:nSims){ #for each simulated experiment
  ...
  z<-t.test(x, mu=100,conf.level = 0.95) #perform the t-test against mu (set to value you want to test against)
  p[i]<-z$p.value #get the p-value and store it
}

#Check power by summing significant p-values and dividing by number of simulations
print(sum(p < 0.05)/nSims) #power
...
axis(side=2, at=seq(0,nSims, nSims/40), labels=seq(0,nSims, nSims/40), las=2)
