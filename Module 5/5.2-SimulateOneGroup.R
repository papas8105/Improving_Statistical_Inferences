...
#Simulate one group
n=10 #set sample size n = 10

x<-rnorm(n = n, mean = 100, sd = 15) #create sample from normal distribution sd = 15


#plot data
g <- ggplot(as.data.frame(x), aes(x))  + 
  ...
  annotate("text", x = mean(x), y = 0.02, label = paste("Mean = ",round(mean(x)),"\n","SD = ",
                                                        round(sd(x)),sep=""),size=8)
# If the plot isn't displayed enter > g
#? Daniel Lakens, 2016. 
...
# Q1 
# > y <- numeric(10)
# > for(ii in 1:10){y[ii] <- mean(rnorm(10,100,15))}
# > max(y) - min(y)
# Q2
# > for(ii in 1:10){y[ii] <- mean(rnorm(10,100,2))}
# > max(y) - min(y)
# Q3
# > g ---->for n = 100